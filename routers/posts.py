...
from db.posts_db import create_post, delete_post, get_all_posts, get_specfic_post, update_post
...
# READ
@router.get("/{user_id}", status_code=status.HTTP_200_OK, response_model=list[PostModelDisplay])
async def get_user_posts(user_id: int, db: Session = Depends(get_db)):
    return get_all_posts(db, user_id=user_id)

@router.get("/post/{post_id}", status_code=status.HTTP_200_OK, response_model=PostModelDisplay)
async def get_user_post(post_id: int, db: Session = Depends(get_db)):
    return get_specfic_post(db, post_id)

# UPDATE
@router.put("/post/{post_id}", status_code=status.HTTP_200_OK, response_model=PostModelDisplay)
async def update_a_post(post_id: int, request: UpdatePostModel, db: Session = Depends(get_db)):
